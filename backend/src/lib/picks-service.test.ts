...
describe("picks-service", () => {
  ...
  it("retrieves pick by ID", () => {
    ...
    const found = getPickById(created.id);
    expect(found).toBeTruthy();
    expect(found?.id).toBe(created.id);
  });

  it("returns null for non-existent pick", () => {
    expect(getPickById("non-existent")).toBeNull();
  });
  ...
  it("filters by unsettled_only", () => {
    ...
    const unsettled = getAllPicks({ unsettled_only: true });
  ...
  it("filters by team (case-insensitive)", () => {
    ...
    const results = getAllPicks({ team: "juventus" });
  ...
  it("updates closing line", () => {
    ...
    const updated = updateClosingLine(created.id, 1.85, "admin");
    expect(updated).toBeTruthy();
  ...
  it("settles a result and calculates P&L", () => {
    ...
    const settled = settleResult(created.id, "won", "admin");
  ...
  it("deletes a pick", () => {
    ...
    const success = deletePick(created.id);
    expect(success).toBe(true);
    expect(getPickById(created.id)).toBeNull();
  });

  it("returns false when deleting non-existent pick", () => {
    expect(deletePick("non-existent")).toBe(false);
  });

  it("batch creates picks", () => {
    const results = batchCreatePicks([
      {
        match_date: "2026-07-01T19:00",
        home_team: "Celtic",
        away_team: "Rangers",
  ...
  it("batch updates closing lines", () => {
    ...
    const results = batchUpdateClosingLines(
      [
  ...
  it("batch settles results", () => {
    ...
    const results = batchSettleResults(
      [
