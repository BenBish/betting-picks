...
export const CreatePickSchema = z
  .object({
    ...
    recommended_odds: z
      .number()
      .gt(1, "recommended_odds must be greater than 1"),
    closing_odds: z
      .number()
      .gt(1, "closing_odds must be greater than 1")
      .optional(),
    stake: z
      .number()
      .min(0, "stake must be greater than or equal to 0")
      .default(1),
...
export const UpdatePickSchema = z
  .object({
    source: z.string().trim().optional(),
  ...
  .refine(
    (data) =>
      !(
        data.home_team &&
        data.away_team &&
        data.home_team.toLowerCase() === data.away_team.toLowerCase()
      ),
    {
      message: "home_team and away_team cannot be the same",
      path: ["away_team"],
